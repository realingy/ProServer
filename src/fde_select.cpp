...
bool Slevents::isset(int fd, int flag)
{
    struct Fdevent *fde = get_fde(fd);
    return (bool)(fde->s_flags & flag);
}

int Slevents::set(int fd, int flags, int data_num, void *data_ptr)
{
    if(fd > FD_SETSIZE - 1){
        return -1;
    }
...
int Slevents::clr(int fd, int flags)
{
    struct Fdevent *fde = get_fde(fd);
    if(!(fde->s_flags & flags)){
        return 0;
...
const Fdevents::events_t* Slevents::wait(int timeout_ms)
{
    struct timeval tv;
    struct Fdevent *fde;
    int i, ret;
